...
void PWM2_CH1_set(int32_t fre, int duty)  //fre为32位int
{
    ...
    if (fre <= 0 || duty < 0 || duty > 100) 
		...
		arr = (int)((72000000.0/(psc * fre))+0.5)-1;		//float强转int提高精度 四舍五入
		
		if (psc >= 65536 || arr <= 0) return; // 超出范围
...
void PWM2_CH2_set(int fre, int duty)
{
    ...
    int psc = 71;
    // 检查频率和占空比参数是否合法
    if (fre <= 0 || duty < 0 || duty > 100) return; // 避免非法参数
    ...
    // 检查 ARR 是否超出范围
    if (arr < 0 || arr > 65535) return; // 超出定时器范围
